...
if (cmd === "test") {
  ...
  const mapping = {
    unit: ["bun", ["run", "unit"]],
    integration: ["bun", ["run", "integration"]],
    e2e: ["bun", ["run", "e2e"]],
    process: ["bun", ["run", "e2e:process"]],
    chaos: ["bun", ["run", "e2e:process:chaos"]]
  };
  if (!target) {
...
if (cmd === "mcp") {
  const mcpPath = join(__dirname, "..", "..", "overdo-mcp", "bin", "overdo-mcp-server.mjs");
  runCommand(process.execPath, [mcpPath]);
  process.exit(0);
}
